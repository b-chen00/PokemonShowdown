...
# Creates POKEMON table if it doesn't exist
...
if c.fetchone()[0] < 1:
    ...
    # read pokemon data from api but only up to generation 5
    req = Request('https://pokeapi.co/api/v2/pokemon/?offset=0&limit=649', headers={'User-Agent': 'Mozilla/5.0'})
    pokeAPI = urllib.request.urlopen(req)
    pokeResponse = pokeAPI.read()
    pokeData = json.loads(pokeResponse)
    # selecting specific information of each Pokemon and inserting it into the table
...
# Creates MOVES table if it doesn't exist
...
if c.fetchone()[0] < 1:
    ...
    # read pokemon move data from api but only up to generation 5
    req = Request("https://pokeapi.co/api/v2/move/?offset=0&limit=728", headers={'User-Agent': 'Mozilla/5.0'})
    movesAPI = urllib.request.urlopen(req)
...
# Creates ABILITIES table if it doesn't exist
